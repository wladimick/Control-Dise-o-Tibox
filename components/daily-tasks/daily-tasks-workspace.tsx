...
export function DailyTasksWorkspace({ tasks, members, workItems, role }: { tasks: DailyTask[]; members: TeamMember[]; workItems: WorkItem[]; role: AppRole }) {
  ...
  return <>
    <PageHeader eyebrow="Bitácora operativa" title="Tareas diarias" description="Registra lo realizado por persona, las HH reales y su asociación con proyectos o requerimientos principales." actions={canEdit(role) ? <button className="tbx-button-primary" onClick={() => setEditing("new")}><Plus size={17}/> Agregar tarea</button> : undefined} />

...
export function WorkItemsTable({
  ...
  const [visible, setVisible] = useState<Record<ColumnKey, boolean>>(() => {
    ...
    try { return { ...defaultVisibility(), ...JSON.parse(saved) } as Record<ColumnKey, boolean>; }
    catch { return defaultVisibility(); }
  });
  const [menuOpen, setMenuOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);

  useEffect(() => { window.localStorage.setItem(storageKey, JSON.stringify(visible)); }, [visible]);
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      return matchesSearch && (type === "all" || item.type === type) && (status === "all" || item.status === status) && (report === "all" || item.report_to_cesar === (report === "yes"));
    });
    ...
    <div>
      <div className="mb-4 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="grid flex-1 gap-2 sm:grid-cols-2 xl:grid-cols-[minmax(240px,1fr)_180px_180px_170px]">
          <label className="relative"><Search className="absolute left-3 top-3 text-[var(--tbx-text-subtle)]" size={17} /><input className="tbx-input pl-10" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar cliente, título o ticket…" /></label>
          <select className="tbx-input" value={type} onChange={(event) => setType(event.target.value)}><option value="all">Todos los tipos</option>{Object.entries(TYPE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
          <select className="tbx-input" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">Todos los estados</option>{Object.entries(STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
          <select className="tbx-input" value={report} onChange={(event) => setReport(event.target.value)}><option value="all">Todos</option><option value="yes">Reportar a César</option><option value="no">Solo internos</option></select>
        </div>
        <div ref={menuRef} className="relative">
          ...
          {menuOpen ? <div className="absolute right-0 top-12 z-20 w-64 rounded-xl border border-[var(--tbx-border)] bg-[var(--tbx-surface)] p-3 shadow-xl"><div className="mb-2 flex items-center justify-between"><span className="tbx-mono text-[.68rem] uppercase tracking-[.12em] text-[var(--tbx-text-subtle)]">Mostrar columnas</span><Settings2 size={15} /></div><div className="max-h-80 space-y-1 overflow-auto">{columns.map((column) => <label key={column.key} className="flex cursor-pointer items-center gap-3 rounded-lg px-2 py-2 hover:bg-[var(--tbx-surface-2)]"><input type="checkbox" checked={visible[column.key]} onChange={() => setVisible((current) => ({ ...current, [column.key]: !current[column.key] }))} /><span className="text-sm text-[var(--tbx-text)]">{column.label}</span></label>)}</div><button className="tbx-button-ghost mt-2 w-full" type="button" onClick={() => setVisible(defaultVisibility())}>Restablecer</button></div> : null}
        </div>
      </div>
      ...
      <div className="tbx-table-scroll"><table className="tbx-table"><thead><tr>
        {visible.code && <th>Código</th>}{visible.client && <th className="sticky-left">Cliente</th>}{visible.title && <th>Trabajo</th>}{visible.type && <th>Tipo</th>}{visible.size && <th>Tamaño</th>}{visible.status && <th>Estado</th>}{visible.source && <th>Origen</th>}{visible.dates && <th>Fechas</th>}{visible.duration && <th>Duración</th>}{visible.weekly && <th>HH sem.</th>}{visible.total && <th>HH total</th>}{visible.assignments && <th>Asignación</th>}{visible.report && <th>Reporte</th>}{visible.updated && <th>Actualización</th>}{visible.actions && <th>Acciones</th>}
      </tr></thead><tbody>
        {filtered.map((item) => <tr key={item.id} className={onOpenItem ? "cursor-pointer" : undefined} tabIndex={onOpenItem ? 0 : undefined} onClick={(event) => { if (!onOpenItem || (event.target as HTMLElement).closest("a,button,input,select,form")) return; onOpenItem(item); }} onKeyDown={(event) => { if (event.key === "Enter" && onOpenItem) onOpenItem(item); }}>
          ...
          {visible.actions && <td><div className="flex items-center gap-1">
            <Link className="tbx-button-ghost" href={`/trabajo/${item.id}/editar`} title={canEdit(role) ? "Editar todo" : "Ver detalle"}><Pencil size={16} /></Link>
            ...
            {role === "admin" ? <form action={deleteWorkItemAction} onSubmit={(event) => { if (!window.confirm("¿Eliminar este registro?")) event.preventDefault(); }}><input type="hidden" name="id" value={item.id} /><button className="tbx-button-ghost text-[var(--tbx-danger)]" type="submit" title="Eliminar"><Trash2 size={16} /></button></form> : null}
          </div></td>}
        </tr>)}
